# -*- coding: utf-8 -*-
"""Predicting topic of paper - GNN.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ahZMXjRI-IbQT4Dd7H65zPvS3hyaGwHX

**Node Classification with Graph Neural Networks**

**Problem Satement:** *Implementing a graph neural network model for predicting the topic of a paper given its citations.*

#### Introduction

Data that comes from nature is easily captured by graphs for example transportation networks, molecular structure, cities map, social and communication networks etc.
**GNN** aims to build and train neural network for graph datasets. In this notebook we are going to build a model that would predict the topic of the paper given its keywords and citations. The *training data* used in this work is *Cora* dataset which serves as standard benchmark graph datasets for many GNN problems.
...
* 2708 scientific papers (represented as **node** in graph)
* 5429 links (represented as **edge** from one paper to other, if a paper x cites another paper y then there will be an edge between x & y).
* Each node has feature represented as **binary word vector** of size 1433.
...
#download the file cora.tgz using keras utility function
zip_file = keras.utils.get_file(
    fname = "cora.tgz", #name of file to be downloaded
    origin = "https://linqs-data.soe.ucsc.edu/public/lbc/cora.tgz", # URL
    extract = True, #Extract the downloaded archive file
)
data_dir = os.path.join(os.path.dirname(zip_file), "cora") #get the directory of downloaded file and there make a directory named "cora"
...
"""The above table states that paper id 35 is being cited by paper 1033, 103482,...etc."""

#Load the papers content into a Pandas dataframe.
...
# Show number of papers in each subject/topic.
print(papers.subject.value_counts())

"""Convert the paper_id and subjects into zero based indices"""

class_values = sorted(papers["subject"].unique())
papers_ids = sorted(papers["paper_id"].unique())
class_idx = {name:id for id, name in enumerate(class_values)}
paper_idx = {name:idx for idx,name in enumerate(papers_ids)}
...
cora_graph = nx.from_pandas_edgelist(citations.sample(n=1500)) #pass edges (source->target) into networkx
subjects = list(papers[papers["paper_id"].isin(list(cora_graph.nodes))]["subject"]) #filter rows by boolean mask, if node is present select the row and select subject
nx.draw_spring(cora_graph, node_size=15, node_color=subjects)

"""**Split the dataset** into stratified train and test sets (preserve the proportion of different classes in train and test)"""

train_data, test_data = [], []

for _, group_data in papers.groupby("subject"):  #iterate through each group based on subject
  #select approx 50% of the dataset for training.
  random_selection = np.random.rand(len(group_data.index)) <= 0.5 #generate random values with length of group_data in [0,1] and compare it with 0.5 and hence generate a boolean mask
  train_data.append(group_data[random_selection])
  test_data.append(group_data[~random_selection])

train_data = pd.concat(train_data).sample(frac=1) #concatenate all selected train data across groups into a single dataframe and then shuffle the rows
test_data = pd.concat(test_data).sample(frac=1)

print("Train data shape:",train_data.shape)
print("Test data shape:",test_data.shape)

"""The above way of splitting the data ensures that in train and test there is approx 50% of data from each subject.

**Implement Train and Evaluate functionality**
"""
...
"""**Evalutate** the baseline model on test data split."""
...
def generate_random_instances(num_instances):
  ...
  for _ in range(num_instances):
    ...
    instance = (probabilities <= token_probability).astype(int) # randomly generated probability is less than or equal to the token probability
    instances.append(instance)

  return np.array(instances)

def display_class_probabilities(probabilites):
  for instance_idx, probs in enumerate(probabilites):
...
# create an edge array (sparse adjacency matrix) of shape [2, num_edges]
